...
osadzonym na Poligonie Northgate.
...
![Architektura OPERATION SENTINEL](SENTINEL%20architecture.png)
...
├── SENTINEL architecture.png        # Wyrenderowany diagram
├── SENTINEL_Scenario_Demo.docx      # Pełny scenariusz demo (Word)
├── SENTINEL_Scenario_Demo.pptx      # Slajdy do prezentacji
├── SENTINEL_Setup_Guide_v2.docx     # Krok-po-kroku setup w Fabric (najnowsza)
├── SENTINEL_Setup_Guide.docx        # Wcześniejsza wersja setup guide
│
├── generate_datasets.py               # Generator 7 strumieni JSONL (seed=42)
├── generate_SENTINEL.py             # Generator dokumentu .docx scenariusza
...
Pełna instrukcja krok-po-kroku znajduje się w **`SENTINEL_Setup_Guide_v2.docx`**.

W skrócie:

1. **Utwórz Workspace** w Microsoft Fabric (z capacity F2+).
2. **Lakehouse** `SENTINEL_Lakehouse` → wgraj 7 plików JSONL z `datasets/`.
...
|---|---|---|
| 🎖️ **SENTINEL Intel** | RadarDetections + DroneObservations + SigintIntercepts | *„Ile hostile tracks w sektorze Bravo w ostatniej godzinie?"* |
| 📦 **SENTINEL Supply** | AmmoLogistics + VehicleStatus + WeatherData | *„Które pojazdy mają paliwo poniżej 30%?"* |
| 🩺 **SENTINEL Medic** | SoldierHealth + VehicleStatus | *„Wskaż żołnierzy z HR > 180 w ostatnich 5 minutach."* |
