"""
OPERATION SENTINEL – Step-by-step Fabric setup guide (DOCX)
"""
...
def create_guide():
    ...
    nb(doc, ' „SENTINEL-Demo"', 'Nazwa:')
    nb(doc, ' (opcjonalnie) „Operation SENTINEL – Fabric RTI battlefield demo"', 'Opis:')
    ...
    nav(doc, 'Workspace SENTINEL-Demo → + New item → wyszukaj „Eventhouse"')

    nb(doc, ' „SENTINEL_Eventhouse"', 'Nazwa:')
    nb(doc, 'Kliknij Create')
    nb(doc, 'Po utworzeniu automatycznie otworzy się widok Eventhouse')
    nb(doc, 'W lewym panelu zobaczysz KQL Database „SENTINEL_Eventhouse" – to Twoja główna baza')
    ...
    nav(doc, 'Eventhouse → kliknij KQL Database „SENTINEL_Eventhouse" → Explore your data (KQL Queryset)')
    ...
    nb(doc, ' „SENTINEL-sensor-input"', 'Source name:')
    nb(doc, 'Kliknij Add')
    nb(doc, 'Po dodaniu, kliknij na source „SENTINEL-sensor-input" na canvasie')
    ...
    nb(doc, ' SENTINEL-Demo (Twój workspace)', 'Workspace:')
    nb(doc, ' SENTINEL_Eventhouse', 'Eventhouse:')
    nb(doc, ' SENTINEL_Eventhouse (domyślna KQL Database)', 'KQL Database:')
    ...
    tbl(doc,
        ...
        [
            ['SENTINEL-Radar', 'radar-input', 'RadarDetections'],
            ['SENTINEL-Vehicle', 'vehicle-input', 'VehicleStatus'],
            ['SENTINEL-Soldier', 'soldier-input', 'SoldierHealth'],
            ['SENTINEL-Drone', 'drone-input', 'DroneObservations'],
            ['SENTINEL-Weather', 'weather-input', 'WeatherData'],
            ['SENTINEL-SIGINT', 'sigint-input', 'SigintIntercepts'],
            ['SENTINEL-Logistics', 'logistics-input', 'AmmoLogistics'],
        ]
    )
    ...
    nb(doc, ' „SENTINEL_Lakehouse"', 'Nazwa:')
    nb(doc, 'Kliknij Create')
    ...
    tip(doc, 'Ścieżka do plików w notebooku: '
        'abfss://SENTINEL-Demo@onelake.dfs.fabric.microsoft.com/SENTINEL_Lakehouse.Lakehouse/Files/')
    doc.add_page_break()
    ...
    nb(doc, ' „SENTINEL_DataSender"', 'Nazwa:')
    ...
    code(doc,
        ...
        'LAKEHOUSE_PATH = "abfss://SENTINEL-Demo@onelake.dfs.fabric.microsoft.com/SENTINEL_Lakehouse.Lakehouse/Files/"\n'
        '\n'
    ...
    nb(doc, ' „SENTINEL – Battlefield COP"', 'Nazwa:')
    nb(doc, 'Kliknij Create')

    h(doc, '7.2 Podłącz data source', 2)
    nav(doc, 'Dashboard → + New data source → OneLake data hub → wybierz SENTINEL_Eventhouse')
    ...
    h(doc, '8.1 Utwórz Data Agent – „SENTINEL Intel"', 2)
    nav(doc, 'Workspace → + New item → wyszukaj „Data Agent" → Create')
    nb(doc, ' „SENTINEL Intel"', 'Nazwa:')
    ...
    nb(doc, 'Wybierz SENTINEL_Eventhouse → SENTINEL_Eventhouse (KQL DB)')
    ...
    code(doc,
        'Jesteś agentem wywiadu wojskowego SENTINEL Intel.\n'
    ...
    tbl(doc,
        ...
        [
            ['SENTINEL Supply', 'VehicleStatus, AmmoLogistics, WeatherData',
             'Agent logistyczny: stan amunicji/paliwa, ETA konwojów, gotowość bojowa'],
            ['SENTINEL Medic', 'SoldierHealth, VehicleStatus',
    ...
    nb(doc, ' „SENTINEL Threat Response"', 'Nazwa:')
    ...
    nav(doc, 'Agent config → Knowledge source → wybierz SENTINEL_Eventhouse (KQL DB)')
    ...
    b(doc, '☐ Teams kanał „SENTINEL-OPS" z powiadomieniami')
    ...
    nb(doc, 'Po ~2 min: otwórz Data Agent SENTINEL Intel → zadaj pytanie testowe')
    ...
    nb(doc, 'Namespace: „SENTINEL-eventhub-ns" (globalnie unikalna nazwa)')
    nb(doc, 'Pricing tier: Basic (wystarczający dla demo)')
    nb(doc, 'Po utworzeniu namespace: + Event Hub → Nazwa: „SENTINEL-events"')
    ...
    code(doc,
        ...
        'EVENT_HUB_NAME = "SENTINEL-events"  # potrzebny przy Azure Event Hub\n'
        '\n'
        'producer = EventHubProducerClient.from_connection_string(\n'
        '    conn_str=EVENT_HUB_CONNECTION_STRING,\n'
    ...
    # ── Save ──
    fname = 'SENTINEL_Setup_Guide_v2.docx'
